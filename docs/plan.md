...
> baseline; `v0.3.0` adds ComfyUI-first generation through both the CLI and Web UI; `v0.4.0` adds
> the optional gzip envelope.
>
...
- [ ] `prototypeWebUI/` still has no automated browser or live-provider test. Offline request-path
      tests cover direct-file access, validation, configuration, and generator request assembly,
      but live provider integration remains a manual check.
