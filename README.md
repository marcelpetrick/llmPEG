...
llmPEG is an experimental **semantic image codec**: a vision model turns an image into a
...
> llmPEG does not preserve pixels. It throws away the original visual data and keeps a
...
[`.llmpeg.json` artifact](examples/news-article.llmpeg.json) came from the local Qwen3-VL model
...
valid schema-constrained JSON in `message.thinking` despite `think:false`; llmPEG accepts that
...
language data. This limitation is explicit: llmPEG consumes OCR text but does not pretend to
ship an OCR engine.
